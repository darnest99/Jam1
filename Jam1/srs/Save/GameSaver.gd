...
const SaveGame = preload('res://srs/Save/SaveGame.gd')
# TODO: Use project setting to save to res://debug vs user://
var SAVE_FOLDER: String = "res://srs/Save/save"
...
func load(id: int):
	...
	print(get_tree().get_nodes_in_group('save'))
	#player base is not within the tree from main menu so it is not in the group
	for node in get_tree().get_nodes_in_group('save'):
		node.load(save_game)
